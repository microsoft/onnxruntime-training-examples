...
def main(args):
    ...
    if args.action == 'download':
        ...
        if not os.path.exists(directory_structure['hdf5'] + "/" + args.dataset + '/training'):
            os.makedirs(directory_structure['hdf5'] + "/" + args.dataset + '/training')

        if not os.path.exists(directory_structure['hdf5'] + "/" + args.dataset + '/test'):
            os.makedirs(directory_structure['hdf5'] + "/" + args.dataset + '/test')

        last_process = None

        def create_record_worker(filename_prefix, shard_id, output_format='hdf5', split='training'):
            bert_preprocessing_command = 'python3 /bert_ort/rakoll/bio-bert/code/DeepLearningExamples/PyTorch/LanguageModeling/BERT/create_pretraining_data.py'
            bert_preprocessing_command += ' --input_file=' + directory_structure['sharded'] + '/' + args.dataset + '/' + split + '/' + filename_prefix + '_' + str(shard_id) + '.txt'
            bert_preprocessing_command += ' --output_file=' + directory_structure['hdf5'] + '/' + args.dataset + '/' + split + '/' + filename_prefix + '_' + str(shard_id) + '.' + output_format
            bert_preprocessing_command += ' --vocab_file=' + args.vocab_file
            bert_preprocessing_command += ' --do_lower_case' if args.do_lower_case else ''
            bert_preprocessing_command += ' --max_seq_length=' + str(args.max_seq_length)
            bert_preprocessing_command += ' --max_predictions_per_seq=' + str(args.max_predictions_per_seq)
        ...
        for i in range(args.n_training_shards):
            last_process = create_record_worker(output_file_prefix + '_training', i, 'hdf5', 'training')

        last_process.wait()

        for i in range(args.n_test_shards):
            last_process = create_record_worker(output_file_prefix + '_test', i, 'hdf5', 'test')

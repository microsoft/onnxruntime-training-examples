...
def main():
    ...
    # 1. Detecting last checkpoint and eventually continue from last checkpoint
    ...
    #3. Load config, tokenizer and model for T5-small
    ...
    # 4. Resizes input token embeddings matrix of the model if new_num_tokens != config.vocab_size and make sure start token is set correctly to begining of sentence
    ...
    # if tokenizer is multi lingual, it sets the source and target language.
    ...
    # 5. Gets the column names for input/target.
    ...
    # 6. Maps the preprocess to all data points
    ...
    # 7. Data collators to form a batch by using a list of dataset elements as input. 
    ...
    my_logs = os.path.join(os.environ["PWD"], training_args.output_dir, "runs" )

    # 9. Custom HuggingFace trainer callback used for starting/stopping the pytorch profiler
    ...
    # 10. Initialize with OnnxRuntime Trainer
    ...
    if training_args.do_train:
        ...
        if int(rank) == 0:
            trained_model_path = Path(training_args.output_dir)

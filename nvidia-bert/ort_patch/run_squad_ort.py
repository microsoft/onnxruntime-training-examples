...
def main():
    ...
    model_state_dict = torch.load(args.init_checkpoint, map_location='cpu')["model"]
    new_dict = OrderedDict((key.replace('Moment_1_model_', '').replace('model_.', '') , value) for key, value in model_state_dict.items() 
            if not (("Moment" in key) or ("_fp16" in key) or ("predictions" in key) or ("seq_relationship" in key) or ("Update_Count" in key)  or "Step" in key))

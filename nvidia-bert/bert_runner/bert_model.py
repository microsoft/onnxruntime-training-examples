...
class BertEncoder(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, output_all_encoded_layers=True):
        ...
        if not output_all_encoded_layers or self._checkpoint_activations:
            all_encoder_layers.append(hidden_states)
...
class BertPreTrainingHeads(nn.Module):
    ...
    def forward(self, sequence_output, pooled_output, masked_lm_labels):
        if self.dense_seq:
            ...
            sequence_indices = torch.nonzero(masked_lm_labels.view(-1) != -1, as_tuple=False).squeeze()
            sequence_output = torch.index_select(sequence_output.view(-1, self.hidden_size), 0, sequence_indices)          
        prediction_scores = self.predictions(sequence_output)
        seq_relationship_score = self.seq_relationship(pooled_output)
